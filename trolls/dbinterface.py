...
def get_items():
    ...
    for i in connection.execute('SELECT * FROM Item'):
        name = i[1]
        type_id = i[2]
        i_type = item.ItemTypes(connection.execute('SELECT * FROM ItemType WHERE Id = "%s"' % str(type_id)).fetchone()[2])
        level = i[3]
        mods_id = i[4]
        ...
        pri_dmg_type_id = i[8]
        pri_dmg_row = connection.execute('SELECT * FROM DamageTypes WHERE Id = "%s"' % str(pri_dmg_type_id)).fetchone()\
            if pri_dmg_type_id is not None else None
        pri_dmg_type = damage.Damage(pri_dmg_row[2]) if pri_dmg_row is not None else None

        sec_dmg_type_id = i[9]
        sec_dmg_row = connection.execute('SELECT * FROM DamageTypes WHERE Id = "%s"' % str(sec_dmg_type_id)).fetchone()\
        ...
        ter_dmg_row = connection.execute('SELECT * FROM DamageTypes WHERE Id = "%s"' % str(ter_dmg_type_id)).fetchone()\
